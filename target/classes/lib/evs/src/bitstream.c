...
)
{
    ...
    if (total_brate == SID_1k75 && sid_update == 1)
    {
        ...
        for (k = 0; k < num_bits; ++k)
        {
            sum += (pt_stream[k / 8] >> (7 - (k % 8))) & 0x1; /*   check of 35 zeroes    */
        }

        if( sum == 0 )
        {
            /* all zeros */
            sid_upd_bad = 1;   /* initial signal as corrupt   */
        }
    }
    ...
    */
    ...
    bit0 = num_bits ? (((pt_stream[0] >> 7) & 0x1) ? G192_BIN1 : G192_BIN0) : 0;
    ...
    if ( st->bfi != 1 )
    {
        ...
        /* for an incorrect total_brate, function decoder_selectCodec can set st->bfi to 1 and keep an incorrect total_brate,  */
    }

    num_bits = total_brate/50;   /* recalc in case total_brate was set to a zeroed good frame by RXDTX handler logic */
    assert( num_bits*50 == total_brate); /* only rates matching 20 ms frames are possible */
