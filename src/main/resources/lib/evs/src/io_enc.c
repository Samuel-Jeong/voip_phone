...
)
{
    ...
    *noDelayCmp = 0;
    ...
    if( i < argc-1 )
    {
        ...
        //if ( (*f_input = fmemopen(src_data, src_data_len, "rb")) == NULL )
        //{
    ...
    if( i < argc )
    {
        ...
        if ( (*f_stream = open_memstream( &dst_data, dst_data_len )) == NULL )
        //char buf[src_data_len];
        //if ( (*f_stream = fmemopen(buf, sizeof(buf), "w+b")) == NULL )
        {
